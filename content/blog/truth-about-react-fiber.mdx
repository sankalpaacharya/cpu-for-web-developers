...
<img src="/blog/fiber/macro.svg" alt="React UI Runtime" width="800" height="100" style={{ display: 'block', margin: '2rem auto' }} />
...
<img src="/blog/fiber/eventloop.png" alt="React UI Runtime" width="800" height="100" style={{ display: 'block', margin: '2rem auto' }} />

A time when it was 2016 react used to render everything in sync, event, buttons clicks, rendering  
everything had a equal priority. 
...
React takes the baton → starts processing a fiber node (or a chunk of work)

After a short time slice, React yields → posts a continuation via MessageChannel

Call stack empties → browser gets control

Browser can paint the UI

Browser can handle clicks, scrolls, input events, timers

If nothing urgent happened, the continuation posts back → React resumes next fiber node

If a new event happened (like a button click that triggered state update) → scheduler now sees a high-priority task → may preempt old fiber

So it’s React → Browser → React → Browser, in small chunks.
